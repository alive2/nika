...
import { getDeepSeekTokenCost } from './pricing.js';
 ...
 * workspace, and day.
 *
 * Ported from alive2/nika-code v1.3.0 `nikaUsageTracker.ts` (MIT).
 */
...
/**
 * The exact usage payload we emit (and parse back) as a `'usage'` data part.
 * Mirrors DeepSeek's `prompt_cache_hit_tokens` / `reasoning_tokens` fields so
 * cost computation can split cache-hit vs cache-miss input legs.
 */
export interface NikaUsagePayload {
...
/** True when a parsed object looks like our `'usage'` data-part payload. */
export function isNikaUsagePayload(value: unknown): value is NikaUsagePayload {
	if (typeof value !== 'object' || value === null) {
		return false;
	}
	const v = value as Record<string, unknown>;
	return typeof v.prompt_tokens === 'number'
 ...
 * usage that arrives as a `'usage'` data part at the end of a request.
 */
export class TokenTrackingProgress implements vscode.Progress<vscode.LanguageModelResponsePart> {
	private _liveChars = 0;
	private _exactUsage: NikaUsagePayload | undefined;

	constructor(
		private readonly _delegate: vscode.Progress<vscode.LanguageModelResponsePart>,
	...
	get exactUsage(): NikaUsagePayload | undefined {
